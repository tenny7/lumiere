...
export default async function OrderDetailPage({
  ...
  const address = order.shipping_address_id
    ? (await supabase.from("addresses").select("*").eq("id", order.shipping_address_id).single()).data
    : null
        ...
        <div className="mb-10">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            {order.payment_status && (
              <span
                className={`inline-flex px-2 py-0.5 text-[0.6rem] font-medium tracking-[0.1em] uppercase border ${PAYMENT_COLORS[order.payment_status] || PAYMENT_COLORS.pending}`}
              >
              ...
              >
                <div className="flex-1 min-w-0">
                  {item.product?.slug ? (
                    ...
                    <span className="font-serif text-sm">{item.product_name}</span>
                  )}
            ...
            <div className="bg-[#1a1918] border border-white/[0.03] p-5">
              <div className="flex items-start gap-3">
                ...
                <div className="text-sm">
                  ...
                  <p className="text-[#8a8478]">{address.line1}</p>
                  {address.line2 && (
                    <p className="text-[#8a8478]">{address.line2}</p>
                  )}
